...
def analyze_reconciled_suggestion(
    *,
    ...
    for index, suggested in enumerate(reconciled.suggestion.units):
        ...
        unit_ev = _unit_ev(unit, suggested.model_count, suggested.combined_from_count)
...
def _suggested_unit_points(
    ...
    upgrade_cost = sum(option.cost for option in _upgrade_options_by_id(unit, selected_upgrade_ids))
...
def _unit_ev(unit: Unit, model_count: int, combined_from_count: int) -> dict[str, Any]:
    ...
    for slot in unit.weapon_slots.all():
        ...
        special_rules = {**(unit.special_rules or {}), **(weapon.special_rules or {})}
